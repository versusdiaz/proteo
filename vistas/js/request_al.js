...
function listar(){
    tabla=$('#tbllistado').dataTable(
	{
		...
		"ajax":
				{
					url: 'controllers/request_al.php?op=listar',
					type : "get",
					dataType : "json",
					error: function(e){
...
function mostrar(idrequest_al){
     $.post("controllers/request_al.php?op=mostrar",{idrequest_al:idrequest_al},function(data,status){
          /*Convertir la cadena enviada desde PHP a un vector de objetos en JavaScript*/
         data = JSON.parse(data);
         mostrarform(true);
         $("#idrequest_al").val(data.idrequest_al);
 ...
 function convertirPresupuesto(){
    var formData = new FormData($("#formulario")[0]);
     $.ajax({
        url:"controllers/request_al.php?op=convertirPresupuesto",
        type:"POST",
        data: formData,
        contentType: false,
	    processData: false,
...
function convertirCompra(){
    var formData = new FormData($("#formulario")[0]);
     $.ajax({
        url:"controllers/request_al.php?op=convertirCompra",
        type:"POST",
        data: formData,
        contentType: false,
	    processData: false,
...
function imprimir(idrequest, idrequest_temp){
    ...
    formData.append("bdDepartamento",'request_al'); // NOTA CAMBIAR PARA CADA DPTO
    $.ajax({
